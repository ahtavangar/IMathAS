# IMathAS Question Authoring Reference
...
- Private: Only you can see the question in question searches.

- Allow use by all: Anyone can use the problem, but only you can modify the problem.
 The question will appear in question searches if it's assigned to a library.
 Anyone can add library assignments, but cannot remove assignments.

- Allow use by all and modifications by group: Like above, but
 anyone in your instructor group can modify the question. Use with caution.

- Allow use by all and modifications by all: Like above, but
 anyone can modify the question. In general, do not use this. This is meant for
 temporary use where you're getting help from someone outside your group.

- License: The permission license to apply to the question.

- My libraries assignments: The question libraries you wish to include this question in.

- Question type: The type of question.

- Common control: The coding that defines variables, defines question behavior, and generates
	anything needed for display.
...
- Question text: The actual question display. This section should be HTML based, but
 ...
 handled ok, but use of the HTML &lt; and &gt; are recommended in their place. You can
	use the Toggle Editor button to enable the rich-text editor if that is easier than working in
	raw HTML.

- Detailed solution: An optional place to define a detailed solution that display
	with the answer key, or a popup written example. This section should be HTML based. There are options
	for whether the solution uses the variables from the question, and for whether to show the solution
	as a popup written example, with the "show answer" answer key, or both

- Image files: This provides a place to upload static images (.png, .jpg, .gif) and
	assign them to a variable for use in the question code or question text. Be sure to provide a description for
	accessiblity. If you're only using an image
	in the question text, it can be simpler to Toggle Editor in the question text and use the insert image tool
	there.

- Help buttons: Use these to provide help buttons for videos or links to reading material. These
	will show below the question, and can be toggled off in the assessment settings.

- Use accessible alternative: This is only needed if your question cannot be made accessible
	through normal means. You probably don't need this.
	Read the accessible alternative help section for more details.
...
- An array: $a = array(6,8,10) or $a = [6,8,10]. This is a collection of variables. You can reference the parts using an index (note that arrays are zero-indexed), so $a[0] = 6.

- An associative array: $a = array('x'=>3, 'y'=>5) or $a = ['x'=>3, 'y'=>5]. This is a collection of variables, referenced by keys. You can reference the parts using the key as index so $a['x'] = 3.

- A string: $a = "hi there", or $b = "2/3".
...
#### Other conditional operators and functions
...
#### Conditionals in Question Text

Within the question text, it's possible to have a section of text display based on the value of a variable.
...
is primarily intended for cases where there is a large block of text to control.
...
- uniqid(): Generates a random unique string, useful for giving a unique id to elements.
...
- horizshowarrays(string,array,[string,array]...,[options]): Creates a tabular display of the data in the array(s) as row(s), with the strings
 as row labels. Does not text wrap, so only use for small data sets. Options can be a string = "c", "l", or "r" to center, left-align, or right align data,
 or can be an array with $opts["align"] for alignment, $opts["tablealign"]="center" to center the whole table,
 and $opts["caption"] to add a caption to the table

- showrecttable(data array of arrays,columnlabels,rowlabels,[options]): Creates a tabular display of a 2x2 data table, where $data[0] is the first row of the table.
 The rowlabels and columnlabels are arrays of labels. Options can be a string = "c", "l", or "r" to center, left-align, or right align data,
 or can be an array with $opts["align"] for alignment, $opts["tablealign"]="center" to center the whole table,
 and $opts["caption"] to add a caption to the table.
...
- text: For placing text labels. A string containing at least the
 first four of: "text,x,y,label,color,location,angle,graphassoc"
...
- graphassoc: If this text is a label for a graph, the index in funcstrings
 for the graph it labels. This is used by the accessible alt text.
...
- invertplot($plot): Inverts any drawing on the plot, swapping x and y coordinates of
	every point. Useful in a case where a function can't handle x= function well, this way you can draw it
	using y= function then invert it. Note you should use replacealttext with this, since any auto-generated
	alt text won't be accurate anymore.

- addfractionaxislabels(plot,step,[axis]): Adds labels along the horizontal axis in steps of step to a plot created with showplot.
 Step is a fraction like "1/4", or a pi multiple, like "pi/4" or "2pi". When using this, make sure to set the showplot x-axis label
 spacing larger than the domain to prevent the default labels from showing. Set axis to "y" to add fraction axis labels on the y-axis.

- addlabel(plot,x,y,label,[color,loc,angle,size,alt]): Adds a label to a plot created with showplot.
...
- alt: alt text for the label, to replace the default.

- addlabelabs(plot,x,y,label,[color,loc,angle,alt]): Adds a label to a plot created with showplot. Parameters are the same as with
...
- textonimage(img,text,left,top[,text,left,top,left,...]): Overlays text over an image. Image can be a URL or the variable for an uploaded
 ...
 which will be read to the screenreader instead of the image and any layered text.
	...
	You can put "centerelements" anywhere after the image to have all elements after that be centered at the given coordinates rather than
	have the upper-left corner at those coordinates.

	There is an alternate input format: textonimage(img,array), where the array is an array [text,left,top,text,left,top,...]
...
- makepretynegative(string): Puts parens around negatives is certain cases.
	Changes 3--4 to 3-(-4) and 3+-4 to 3+(-4), but note it will change 3-+4 to 3-4.
...
- fractowords(numerator,denominator,[options]): Creates a string containing
 ...
 If 'over' and 'by' both used, defaults to 'over'. If 'mixed' and 'literal' both used,
	defaults to 'mixed'. 'hyphen' will hyphenate the fraction.

	fractowords(5,-2) returns "negative five halves"

	fractowords(6,3) returns "two"

 fractowrods(2,3,'hyphen') return 'two-thirds'
...
- prettyreal_instring(string,decimals,[comma]): Formats any number in the string
 using the same process as prettyreal.

- round_instring(string,[decimals]): Rounds any number in the string
 to the specified number of decimal places (nearest whole number if decimal is left out)
...
- rawurlencode(string): Encodes a string for use in a URL query string parameter.
...
- stringclean(string,[mode]): mode 0 (default): trims leading and trailing whitespace. mode 1: removes all whitespace. mode 2: removes all non-alphanumeric characters.

- substr(string,start,[length]): grabs the part of the string from the start index with given length.
...
- lcfirst(string): makes the first character in the string lower case
...
- preg_match(pattern,subject,[matches]): Searches subject for a match using the regular expression in pattern. Returns true/false, and stores the match in matches.

- preg_match_all(pattern,subject,[matches, flags, offset]): Searches subject for multiple matches using the regular expression in pattern. Returns number of matches, and stores the matches in matches. See PHP help for optional flags.

- preg_replace(pattern,replacement,subject,[limit]): Searches subject for a match using the regular expression in pattern, and replaces any matches with the replacement.

## Array Macros

These macros are for working with arrays:

- listtoarray(list, [tonum]): Converts a list, like "1,2,3,4" or "cat,dog,rabbit" to an array.
	If the list is decimal or integer numbers, set tonum to true to ensure they're treated like numbers.
...
- arraytolist(array, [space]): Converts an array to a list. Set the second argument to true to add spaces after the commas.

- joinarray(array,[symbol,ksort]): Convert an array to a string, joining elements with the given symbol (default comma). Set ksort to true to sort the array by key before joining, useful if elements were defined out of order.

- stringtoarray(string, [tonum]): Converts a text string to an array of characters.
	If the characters are all numbers, set tonum to true to ensure they're treated as numbers.

- fillarray(value,num,[start]): Creates an array with num entries, all of which are the same given value. Array indexing
 starts at 0 unless specified. Alternatively, num can be an array of indices, in which case the array will
	only be defined to the value at those indices.
...
- arraysetvalues(array,keyarray,value/valuearray):
	Sets values on an existing array, at the keys specified in the keyarray. If value is a single value,
	that value is set at all the keys. This is similar to fillarray with an array for num, but this function
	modifies an existing array rather than creating a new one. If an array of values is provided, the corresponding value is set
	for each key in the keyarray. Returns the array with new values.
...
- print_r(array, [return]): Outputs an array for debugging purposes. Add second argument true to return the output as a string you
	can store to a variable and include in your question text for nicer debugging.
...
- scoremultiorder(stua, answer, swap, type, [weights, options]): In a multipart question, allows questions or groups
...
- options: an array of other options. To use without enabling the weights, set weights to null.

- variables: to change the variables when type is numfunc. Example:
 $answer = scoremultiorder($stuanswers[$thisq],$answer,'0;1','numfunc',null,['variables'=>$variables])

- scoreperiodic(answer, stuanswer, variable, [tolerance]): This helps scoring of "all solutions"
	periodic answers, like "pi/6+2pik" by redefining the $answer if the student's answer is equivalent but
	with a different base value, like "-11pi/6+2pik". It works for a single expression or list of solutions.
	The function returns a value that should replace $answer, so make sure you separately define $showanswer so
	a "nice" answer gets displayed in the key. Here's a couple examples of use:

- Single part: (algebraic expression type)

 $answerformat = "list"

 $variables = "k"

 $answer = "pi/6 + 2pik, pi/3+2pik"

 $showanswer = "`$answer`"

 $answer = scoreperiodic($answer, $stuanswers[$thisq], "k")

- Multipart part (suppose part index 3 is "numfunc"):

 $answer[3] = "pi/12 + pi/3n

 $variables[3] = "n"

 $showanswer[3] = "`$answer[3]`"

 $answer[3] = scoreperiodic($answer[3], getstuans($stuanswers,$thisq,3), "n")

- setupmathquillfillin(asciimath format,alt format): For use in a multipart question,
 this allows you to create fill-in areas inside an otherwise static math expression. Setup a multipart
 question like usual, then use this function to generate an entry string you place into the question text
 instead of $answerboxes for the parts involved. The asciimath string should be the expression to render, with
 [AB#] where the fill-ins will go, corresponding to the part indexes of the multipart question. The alt format
 string should be a plain text string using the same [AB#] inputs, which will be used for students who have
 disabled the equation editor for accessibility. Example of use:

 $entry = setupmathquillfillin("int_([AB0])^([AB1]) [AB2] dx", "The integral from [AB0] to [AB1] of [AB2] dx")

 If you want to define a single answer key for the whole fillin, define $showanswer[2] as
 ...
 want to set $showanswerstyle = "inline" so the key shows on the same line as the fillin entry.

 To adjust the size of fillins, use $answerboxsize[part number] as usual, but be aware large sizes may be
...
- sqrt(t), root(n)(t): Square root and nth root
...
- hexdec(a): converts a hexidecimal string to a decimal number

- dechex(a): converts a decimal number to a hexidecimal string

- v!: Factorial, like $a = $b!

- evalfunc(func,vars,val1,val2,...,[falseonerror]): Evaluates a function given the variables and values for each variable.
 for example evalfunc("x^2*y","x,y",2,3) will evaluate x^2*y with x=2, y=3.

 This function by default will return false on invalid parameters.
 ...
 some additional cleanup steps that sometimes cause problems.
...
- stuansready($stuanswers, $thisq, array of part numbers, [anstypes, answerformat]): Before using
 ...
 only checks Calculated and Calculated N-tuple questions, and no lists). Or you can put 'checknumeric' for the answerformat
 and it will check that "number", "calculated", "ntuple", and "calcntuple" answers are numeric
	(must use $stuanswersval for calculated and calcntuplein this case).
 Note this won't work for list answers, only single numbers, and that it won't recognize answers
 with commas like "2,345" as numeric. For calcmatrix, it's best to pass $stuanswersval, as that will
	allow stuansready to confirm all elements evaluate to a value.
...
- comparecomplex(a,b,[tol]): Compares complex number expressions a and b to see if there are equivalent to the specified
 tolerance. Set tol to specify the relative tolerance (defaults to .001), or prefix with | for an absolute tolerance.
 Note this accomodates any format, so 'sqrt(-4)' and '2i' will be considered equivalent, as would 'e^(pi*i)' and '-1'.
...
- comparesameform(a,b,[vars]): Compares expressions a and b to see if they are
...
- are_numeric(v1,v2,v3,...): Checks if all the arguments are or represent an integer or decimal number. You can also input a single array instead of individual values.
...
* / + - | Multiply, divide, add, subtract | ^ | Powers. 2^3 = 8. | e, pi | The standard constants | % | Modulus of integers (remainder after division. 5%2 = 1) | mod(p,n) | Modulus of integers. This version gives positive results for modulus of negative numbers. | fmod(p,n) | Modulus of decimal values. May give negative results from negative inputs. | ! | Factorial | sqrt | Square root | sin,cos,tan,cot,sinh,cosh | Standard trig function. Be sure to enter as sin(2), not sin 2 | arcsin,arccos,arctan,arcsinh,arccosh | Inverse trig functions. | sin^-1, cos^-1, tan^-1 | Alternative entry for inverse trig functions. Use like sin^-1(0.5) | ln | Natural Logarithm base e | log | Common Logarithm base 10 | abs | Absolute Value. Note that while abs() is used for calculations, you may prefer to use | brackets for display | round(n,d) | round number n to d decimal places | roundsigfig(n,s) | round number n to s significant digits. This is for calculations;
	if you need a value for display and want to ensure trailing 0's display, use prettysigfig | floor,ceil | floor/ceiling: integer below/above given number | min,max | min or max of the passed values. Can be used as min($a,$b,$c) or min($arrayvariable).
...
The accessible alternative option, near the bottom of the question editor, allows you to specify an
	alternative question ID to display if the student has accessibility settings enabled.

You most likely should not use this feature. It is intended for the rare
 ...
 some questions using jsxgraph, Geogebra, etc. If your question uses static
	...
	you DO NOT NEED this feature, and will make things worse by using it.
...
Likewise, the system automatically provides a keyboard-based alternative for drawing questions when
	the student has the keyboard based drawing setting enabled.

It is always preferable to make the original question accessible when possible rather than
 ...
 that assesses equivalent concepts but in a different way.

For the "Use accessible alternative", you can select to have the alternative displayed:

- "visual alt": when the student has Graph Display set to text alternatives. This is the
 ...
 visual in <div aria-hidden=true></div> to hide it from a screenreader, and add
...
- "mouse alt": when the student has Drawing Entry set to Keyboard and text-based. If your question has an
...
- "visual or mouse alt": if the question has both visual elements and elements that require a mouse,
...
- $stulastentry or $stulastentry[P] will contain the student's entered answer on the current question.
 While $stuanswers will only contain submitted answers, $stulastentry will also include autosaved entries.
...
- $graphdispmode: This will be 0 if the student has text-based alternatives
 for graph display enabled, and 1 or 2 if an image-based mode is enabled.

- $drawentrymode: This will be 0 if the student has keyboard-based
 alternatives for drawing entry enabled, or 1 for mouse-based entry.
...
When the "=2" or "r2" form is used, it overrides both $reltolerance and $abstolerance, and by default tests that the student answer exactly matches
	the correct answer ($abstolerance=0).

To override the tolerance use +- in the setting, like this:
...
$reqsigfigs = a number
	...
	$reqsigfigs = "=2" to require exactly 2 sig figs in the answer, or $reqsigfigs = "[2,4]" to require between 2 to 4 sigfigs. This will put also a message in the answer tips stating the
...
$requiretimes = a list, like "E,=0"
	...
	the symbol "E" would be required to show up exactly 0 times.
...
$requiretimeslistpart = a list, in the same format as $requiretimes
	...
	where $requiretimes would be applied to the entire list of answers.
...
- To override the default "no solutions" and "infinite solutions" wording, you can define $ansprompt as a list of semicolon-delimited strings
 to override the default wording. For example,
...
$reqsigfigs = a number
	...
	$reqsigfigs = "=2" to require exactly 2 sig figs in the answer, or $reqsigfigs = "[2,4]" to require 2 to 4 sigfigs. This will put also a message in the answer tips stating the
	significant figures required. If you use the "=2" form, it will also round your $answer if needed for the $showanswer.
...
$partialcredit = array(answer,score,[answer,score,...])
 ...
 and subsequent entries to apply to the $partialcredit answers.
$ansprompt = string
	A string that will be displayed in front of the input box. Example: $ansprompt="y="
$answerboxsize = number
	Determines the number of characters space provided for entry of an answer. Defaults to 20.
...
$ansprompt = string
 This allows you to override the default "-" that displays in the dropdowns when no selection
...
$answerformat = "equation" or "inequality" or "doubleinequality" or "generalcomplex" or "toconst" or "scalarmult" or "nosoln" or "nosolninf" or "sameform" or "list" or "casesensitivevars" (can combine, like "equation,nosoln")
 ...
 on equations that are near zero for most values of the input; this can often be overcome by changing the $domain
	...
	"doubleinequality": Specifies that the answer is expected to be a compound inequality like 3<2x+3≤5 rather than an expression.
	The given answers should also be a compound inequality.
...
- all_words: gives credit the student answer contains a list of key words. $answer
...
$partialcredit = array(string,score,[string,score,...])
 Allows you to specify partial credit for other answers. The array should be
 ...
 string answers; list answers and "all_words" are not supported.
 $ansprompt = string
...
- 3pointparab (parabola given 3 points)
...
- tan (tan and cot function)

- sec (sec and csc function)
...
- For dashed lines, give $answers as "dashedline,function" like "dashedline,2/3x+4"
...
If desired, you can set separate label and grid
...
To specify alt-text for the background, add an element to the background array, like
...
$scoremethod = "takeanything", "direction", "relativelength", "ignoreoverlap", "ignoreextradots"
...
$answerformat = "scalarmult", "anyorder", "nosoln", "nosolninf", "dimensionhint"
 ...
 "dimensionhint" will show a warning to students if they enter the wrong number of components.
...
$displayformat = "point", "pointlist", "vector", "vectorlist", "list", "set", "setlist"
	Changes the answer entry tips (does NOT change how the question is graded). For points,
	entry like (2,3) is specified. For vectors, entry like <2,3> is specified.
$answerformat = "scalarmult", "anyorder", "dimensionhint", "fraction", "reducedfraction", "mixednumber", "scinot", "fracordec", "nodecimal", "notrig", "nosoln", "nosolninf", "sameform"
 "scalarmult" will accept any answer that is a scalar multiple of the correct answer.
...
### Complex N-Tuple
 ...
 $answer = a string containing an n-tuple or list of n-tuples of complex numbers in a+bi form.
 Defines the answer n-tuple or list of n-tuples. N-tuples can be any dimension,
 but must be surrounded by any of: (), [], {}, <>. Examples: $answer = "(1,2+i)",
 $answer = "<i,3+i,4>,<1+i,5-i,7>", $answer = "(1,2i) or (1+i,3)"
 ...
 Changes the answer entry tips (does NOT change how the question is graded). For points,
 entry like (2,3) is specified. For vectors, entry like <2,3> is specified.
 $answerformat = "allowjcomplex", "dimensionhint", "nosoln", "nosolninf"
...
### Calculated Complex N-Tuple

 Calculated Complex N-Tuple questions require the student to enter an n-tuple or list of n-tuples of complex numbers.
 This can be used for coordinate points, vectors, or any other n-tuple of complex numbers.
 ...
 Each complex number must be in a+bi format unless $answerformat="sloppycomplex" or "generalcomplex" is used.
 ...
 Changes the answer entry tips (does NOT change how the question is graded). For points,
 entry like (2,3) is specified. For vectors, entry like <2,3> is specified.
 $answerformat = "sloppycomplex", "generalcomplex", "allowjcomplex", "dimensionhint", "fraction", "reducedfraction", "mixednumber", "scinot", "fracordec", "nodecimal", "notrig", "nosoln", "nosolninf", "sameform"
 Use "sloppycomplex" to allow the student to enter complex numbers in forms other than a+bi
 (allows (4+sqrt(3)i)/5 for example). Student answer must still only contain one i.

 Use "generalcomplex" to allow students to enter complex expressions that may involve calculations, like (e^(3i),4).
 ...
 without decimals (also disallows 10^-2 and 3E-2), or
 an answer without trig functions (sin,cos,tan,sec,csc,cot). Multiple options can be specified like $answerformat="nodecimal,notrig".
 Note that component-based answerformats won't be checked for sloppycomplex or generalcomplex.
...
### Algebraic N-Tuple
 ...
 Algebraic N-Tuple questions require the student to enter an n-tuple or list of n-tuples of algebraic expressions.
 ...
 $answer = "<t,4-t,2+3t>,<t,3t,8t>", $answer = "(x,x^2) or (sqrt(x),x)". .
 ...
 values. The domain defaults to real numbers from -10 to 10. If using multiple variables, can extend domain to
 define domain for each variable separately. Example: $variables = "x,y"; $domain = "0,5,20,25"
 $answerformat = "generalcomplex", "dimensionhint", "nosoln", "nosolninf", "sameform"
 Use "generalcomplex" to allow students to enter complex functions involving i.
 ...
 $requiretimes = a list, like "^,=3,cos,<2"
 Adds format checking to the student's answer. The list can include multiple checks, which come in pairs. The first is the symbol to look for. The second describes what is acceptable.
 See the Function question type for more details. Note the $requiretimes applies to the student's entire answer.

 $requiretimeslistpart = a list, in the same format as $requiretimes
 Similar to $requiretimes, but this check is applied to each ntuple when the answer is a list of ntuples.
 $scoremethod = "byelement"
 By default, the whole n-tuple must be correct to receive any credit.
 Set $scoremethod = "byelement" to give partial credit if only some elements of
 the n-tuple are correct.
...
$displayformat = "det" or "inline" or "augmented"
...
### Complex Numerical Matrix
 ...
 $answer = string describing a matrix of complex numbers
 Defines the answer. Example: $answer = "[(1+i,2,3i),(8,5+i,6)]" is a 2x3 matrix with first row: 1+i,2,3i
 ...
 Defines the size of the answer matrix. If this is supplied, the student will be provided with a grid of
 entry boxes in which to input the matrix. If this is not supplied, they will be required to enter the matrix
 using the ASCIIMath notation, like "[(1+i,2,3i),(8,5+i,6)]" or using the equation editor palette.
...
### Calculated Complex Matrix

 The student is asked to enter a matrix of complex numbers or calculations, like 2/3+1/3i or 5^2.

 Required Variables

 $answer = string describing a matrix of complex numbers, or calculations leading to numbers
 Defines the answer. Example: $answer = "[(1+i,2,3i),(3/2+i,3sqrt(2)i,6)]" is a 2x3 matrix with first row: 1+i,2,3i
 ...
 Defines the size of the answer matrix. If this is supplied, the student will be provided with a grid of
 entry boxes in which to input the matrix. If this is not supplied, they will be required to enter the matrix
 using the ASCIIMath notation, like "[(1+i,2,3i),(3/2+i,3sqrt(2)i,6)]" or use the equation editor palette
 $answerformat = "sloppycomplex", "generalcomplex", "allowjcomplex", "fraction", "reducedfraction", "mixednumber", "scinot", "fracordec", "nodecimal", or "notrig", "nosoln","nosolninf"

 Use "sloppycomplex" to allow the student to enter complex numbers in forms other than a+bi
 (allows (4+sqrt(3)i)/5 for example). Student answer must still only contain one i.

 Use "generalcomplex" to allow students to enter complex expressions that may involve calculations, like e^(3i).
...
### Algebraic Matrix
 ...
 $answer = string describing a matrix of algebraic expressions
 Defines the answer. Example: $answer = "[(x,x^2,3),(4,5+x,2)]" is a 2x3 matrix with first row: x,x^2,3
 ...
 Defines the size of the answer matrix. If this is supplied, the student will be provided with a grid of
 entry boxes in which to input the matrix. If this is not supplied, they will be required to enter the matrix
 using the ASCIIMath notation, like "[(x,x^2,3),(4,5+x,2)]" or use the equation editor palette
 $variables = string
 A list of all variables in the function (including letters representing constants). Defaults to "x". Can use
 "f()" to indicate a symbol represents a function.
 $domain = "inputmin,inputmax"
...
### Chemical Molecule Drawing
 ...
 $answer = "SMILES~~~CML"
 Defines the answer, which includes the SMILES representation of the molecule,
 as well as the CML representation used for displaying the answer key.
 Use our answer generator to
 draw the answer and generate the $answer string. You can specify multiple
 acceptable answers separated by or, like "SMILES~~~CML or SMILES~~~CML"
 ...
 Using the variable $answerbox in the Question Text will place the entry box in that location
 $showanswer
 The answer to show to students (if option if available). Defaults to a drawing of the correct answer.
 Use this to substitute a detailed answer.

### File Upload

A student is asked to upload a file. The file upload type is not automatically graded.
The file must be an allowed file type, one of these extensions: @whitelist@.
...
Question part variables
	...
	Note that if you do not index the variable, it will apply to all parts. See $defaults below for more info and other
	options.
...
$defaults
	To set an optional variable to the same value for every question part, you can simply specify
 it without index, like $reqdecimals=1, and it will apply to all parts.
 ...
 those variables have special meaning in that context.
$showanswer
...
$showanswerloc (In Question Text)
	...
	Show Answer button, and have defined $showanswer[n] for only some parts, you must place [SABn] for all parts,
	or else the remainder will not show. To define a single Show Answer for a sequential set of parts, define
	$showanswer[n] using the last part number in the set, and put [SABn-n] in the question text to place
	the Show Answer button for that set of parts. For example, if you parts 2-4 were to have a single key, you'd define
	$showanswer[4] and put [SAB2-4] in the question text.
$showanswerstyle
	Use $showanswerstyle = 'inline' to
	allow Show Answer buttons placed using $showanswerloc or [SABn] to be placed inline with text rather than on its own line.
[SAB#] (In Detailed Solution)
	To include the generated "show answer" values in the detailed solution, you can use short
 tags like [SAB2]. Note that this will place the "show answer" directly, not wrapped in a
 button like in question text.
...
- The string 'manual': will assign a score of 0 but treat the whole question
 as a manual-grade question.
